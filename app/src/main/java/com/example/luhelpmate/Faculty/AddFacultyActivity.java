...
public class AddFacultyActivity extends AppCompatActivity {
    ...
    private Pattern p = Pattern.compile("([+][8][8])?[0][ ]?[1][0-9]{3}[-]?[0-9]{6}");
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String[] items = new String[]{"Select Designation", "Head", "Professor", "Associate Professor", "Assistant Professor", "Lecturer",};
    ...
    private void checkValidation() {
        name = addFacultyName.getText().toString().trim();
        initial = addFacultyInitial.getText().toString().trim();
        phone = addFacultyPhn.getText().toString().trim();
        email = addFacultyEmail.getText().toString().trim();
        if (name.isEmpty()) {
            addFacultyName.setError("Empty");
            addFacultyName.requestFocus();
        } else if (!n.matcher(name).matches()) {
            addFacultyName.setError("Name can be only Alphabet");
            addFacultyName.requestFocus();
        } else if (initial.isEmpty()) {
            addFacultyInitial.setError("Empty");
            addFacultyInitial.requestFocus();
        } else if (!n.matcher(initial).matches()) {
            addFacultyInitial.setError("Initial can be only Alphabet");
            addFacultyInitial.requestFocus();
        } else if (designation.equals("Select Designation")) {
            Toast.makeText(AddFacultyActivity.this, "Please Select Designation", Toast.LENGTH_SHORT).show();
        return;
